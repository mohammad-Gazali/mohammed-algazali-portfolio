...
import ProjectCardImageSlider from "./ProjectCardImageSlider";
		...
		<AnimatePresence>
			{selectedId && (
				<motion.div
					...
					className="bg-primary-light text-primary-light-foreground fixed inset-0 overflow-y-auto custom-scrollbar flex flex-col items-center sm:gap-3 gap-2 z-50 pb-10"
					layoutId={selectedId}
				>
					<button
					...
					<h3 className="text-3xl font-bold mt-12">{selectedProject?.name}</h3>
					<div className="flex flex-col justify-center w-full mt-10 lg:px-10 gap-6">
						...
						/>
						<p className="leading-7 lg:mx-0 mx-6 inline-block">
							{selectedProject?.description}
						</p>
					</div>
					<h3 className="lg:mx-10 mx-6 self-start sm:text-3xl text-2xl font-medium mt-10">
						Used Technologies
					</h3>
					<ul className="flex flex-wrap gap-4 w-full lg:px-10 px-6 mt-4">
          ...
          <a href="" target=""></a>
          <Button target="_blank" className="mt-10 lg:py-6 sm:py-6 py-6 sm:w-full max-w-md" size="lg" href={selectedProject?.repository} asLink>
